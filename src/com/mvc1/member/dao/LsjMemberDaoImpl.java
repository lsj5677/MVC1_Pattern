...
import com.mvc1.member.sql.LsjMemberQueryMap;
	...
	public boolean updateLsjMem(LsjMemberVO lvo) {
		...
		// insert, update, delete = count에 결과값 담아주기, count=int
				...
				try{
					...
					pstmt.setString(2, lvo.getLhp());
					// 이름으로 찾아서 업데이트
					pstmt.setString(3, lvo.getLname());
		...
		if(lvo!=null){
			try{
				...
				if(rsRs!=null){
					...
					// 결과값이 존재하면 수행
					while(rsRs.next()){
						lvo_ = new LsjMemberVO();
						
						// set으로 초기화 get으로 리턴
						lvo_.setLid(rsRs.getString("LID"));
						lvo_.setLpw(rsRs.getString("LPW"));
